<div id="content-single" class="content group">
    ...
    <div class="hentry hentry-post blog-big group ">
        ...
        <div class="meta group">
            ...
            <p class="categories"><span>In: <a href="{{ route('articles_cat', ['alias' => $article->category->alias ]) }}" title="View all posts in {{ $article->category->title }}" rel="category tag">{{ $article->category->title }}</a> </span></p>
            <p class="comments"><span><a href="#comments" >{{ count($article->comments) ? count($article->comments) : '0' }} {{ Lang::choice('site.comments', count($article->comments)) }}</a></span></p>
        </div>
        <!-- post content -->
    ...
    <div id="comments">
        <h3 id="comments-title">
            <span>{{ count($article->comments) }}</span> {{ Lang::choice('site.comments', count($article->comments)) }}
        </h3>

        @if (count($article->comments) > 0)
        <ol class="commentlist group">
            @foreach ($article->comments->groupBy('parent_id') as $k => $item)

                @if ($k !== 0)
                    @break
                @endif

                @include('pink.comment', ['parent_comments' => $item])
        ...
        <div id="respond">
            ...
            <form action="{{ route('comment.store') }}" method="post" id="commentform">
                ...
                @if (!Auth::check())
                    ...
                    <p class="comment-form-url"><label for="site">Website</label><input id="website" name="website" type="text" value="" size="30" /></p>
                @endif
